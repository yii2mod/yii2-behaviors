...
class NestedSetBehavior extends Behavior
{
    ...
    {
        if ($this->owner->getIsNewRecord() && !empty($this->owner->parent)) {
            $model = get_class($this->owner);
            $root = $model::findOne($this->owner->parent);
            $this->appendTo($root);
        } elseif (!$this->owner->getIsNewRecord()) {
            $root = $this->owner->parent()->one();
            if (empty($this->owner->parent) && !$this->owner->isRoot()) {
                $this->moveAsRoot();
            } elseif ((!isset($root) || ($this->owner->parent !== $root->root))  && !empty($this->owner->parent)) { // && $this->owner->isRoot()
                $model = get_class($this->owner);
                $root = $model::findOne($this->owner->parent);
